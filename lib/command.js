...
module.exports = function(proto) {
  ...
  function args(prop) {
    return function args() {
      ...
      for (; i < len; ++i) {
        ...
        this.emit('flagAdded', arguments[i]);
      }
  ...
  proto._spawn = function _spawn(args, bufferOutput, callback) {
    ...
    var cmd = bin + ' ' + args.map(utils.escape).join(' '),
      self = this,
      proc, err, timeout = parseInt(this._options.timeout),
      timeoutId;
    ...
    try {
      proc = spawn(bin, args);
      //Triggers monitor() output if enabled
      this.emit('processSpawned', proc);
  ...
  proto.args = function args() {
    ...
    return [].concat(
      this._subCommand, this._in, this.src(), this._out, outname
