...
module.exports = function (proto) {
  ...
  proto._spawn = function _spawn (args, bufferOutput, callback) {
    ...
    // There are many unix-like systems (Linux, FreeBsd, Minix, ...), 
    // may be we should let users decide when it is appropriate to use "nice and "ionice" command.
    //if( process.platform == 'unix' || process.platform == 'linux' ){
    		if (this._options.nice) cmd = 'nice ' + this._options.nice + ' ' + cmd;
    		if (this._options.ionice) cmd = 'ionice ' + this._options.ionice + ' ' + cmd;
    //}
