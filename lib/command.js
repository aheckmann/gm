...
module.exports = function (proto) {
  ...
  proto._preProcessors = [];
  proto.preProcess = args('_preProcessors');
  ...
  proto.write = function write (name, callback) {
    ...
    this._preProcess(function(err) {
      ...
      return self._spawn("gm", self.args(), true, callback);
    });
  }
  ...
  proto.stream = function stream (format, callback) {
    ...
    this._preProcess(function(err) {
      if(err) return callback(err);
      return self._spawn("gm", self.args(), false, callback);
    });
  }
  ...
  proto._preProcess = function _preProcess (callback) {
    if(!this._preProcessors.length) return callback();
    // execute preProcessors in series
    var self = this
      , cmds = self._preProcessors.map(function(preProc, i) {
                 return function() {
                   preProc.call(self, function() {
                     var next = cmds[++i];
