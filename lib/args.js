...
module.exports = function (proto) {
  ...
  proto.compose = function compose (operator) {
    // operators: Over,In,Out,Atop,Xor,Plus,Minus,Add,Subtract,
    // Difference,Divide,Multiply,Bumpmap,Copy,CopyRed,CopyGreen,
    // CopyBlue,CopyOpacity,CopyCyan,CopyMagenta,CopyYellow,CopyBlack
    return this.out("-compose", operator || 'Over');
  }
  
  // http://www.graphicsmagick.org/GraphicsMagick.html#details-filter
  proto.filter = function filter (type) {
    // types: Point,Box,Triangle,Hermite,Hanning,Hamming,Blackman,
    // Gaussian,Quadratic,Cubic,Catrom,Mitchell,Lanczos,Bessel,Sinc
    
    // "type" argument must be provided because:
    ...
    // channel, or is being enlarged, otherwise the Lanczos filter is used.
    if(type) return this.out("-filter", type);
    return this;
  }
