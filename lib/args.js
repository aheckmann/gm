...
module.exports = function (proto) {
  ...
  var getArrayArgs=function(){
    var arrArgs=[];
    for (var i = 0; i <= arguments.length; i++) {
      if(typeof arguments[i] != 'undefined') arrArgs.push(arguments[i]);
    }
    return arrArgs;
  };
  ...
  proto.blackThreshold = function blackThreshold (red, green, blue, opacity) {
    return this.out("-black-threshold", getArrayArgs(red, green, blue, opacity).join(','));
  }
  
  // http://www.graphicsmagick.org/GraphicsMagick.html#details-blue-primary
  proto.bluePrimary = function bluePrimary (x, y) {
    return this.out("-blue-primary", getArrayArgs(x, y).join(','));
  }
  ...
  proto.chop = function chop (x, y) {
    return this.out("-chop", getArrayArgs(x, y).join(','));
  }
  ...
  proto.colorize = function coalesce (value) {
    return this.out("-colorize", value);
  }
  ...
  proto.gaussian = function gaussian (radius, sigma) {
    return this.out("-gaussian", getArrayArgs(radius, sigma).join('x'));
  }
  ...
  proto.level = function level (blackPoint, gamma, whitePoint, percent) {
    return this.out("-level", getArrayArgs(blackPoint, gamma, whitePoint).join(',')+(percent?'%':''));
  }
  ...
  proto.shadow = function shadow (radius, sigma) {
    return this.out("-shadow", getArrayArgs(radius, sigma).join('x'));
  }
  ...
  proto.size = function size (width, height, offset) {
    return this.out("-size", width+'x'+height+(typeof offset!='undefined'?'+'+offset:''));
  }
  ...
  proto.whiteThreshold = function whiteThreshold (red, green, blue, opacity) {
    return this.out("-white-threshold", getArrayArgs(red, green, blue, opacity).join(','));
  }
