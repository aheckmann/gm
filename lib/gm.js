    ...
    var parts = stdout.split("\n")
      ...
      , handle = 
        { 'Geometry': function(val){
            ...
            data.size = { width: parseInt(split[0],10), height: parseInt(split[1],10) } 
          }
        , 'Format': function(val){
...
gm.prototype.thumb = function(w, h, name, callback){
  ...
  self.size(function(err, size){
    ...
    if (size.width < size.height){
      ...
      w1 = w,
      h1 = h
    self.arg(["-size", w1+"x"+h1]).scale(w1, h1).crop(w, h).quality(63).name(name).noProfile().write(function(){
