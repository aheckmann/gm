...
gm.prototype.thumb = function(w, h, name, callback){
  // here w and h are changed to be minimum values for thumbnails
  var self = this
  self.size(function(err, size){
    if (err) return callback(arguments)
    w = parseInt(w, 10)
    h = parseInt(h, 10)
    ...
    if (size.width < size.height)
      w1 = w,
      h1 = Math.floor(size.height * (h/size.width))
    else if (size.width > size.height)
      w1 = Math.floor(size.width * (w/size.height)),
      h1 = h
    else if (size.width == size.height)
      w1 = w,
      h1 = h
    
    self.scale(w1, h1).crop(w, h).name(name).noProfile().write(callback)
  })
  return self
