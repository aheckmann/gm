...
module.exports = function (gm) {
  ...
  Object.keys(map).forEach(function (getter) {
    proto[getter] = function (opts, callback) {
      ...
      // wrap to object `identify` format options if string passed
      if (key === 'format' && typeof (opts) === 'string') {
        opts = {
          custom: opts
        };
      }
      ...
      var args = makeArgs(self, val, opts);
      self._exec(args, function (err, stdout, stderr, cmd) {
        ...
        if (val.helper in helper) {
          helper[val.helper](self.data, result, opts);
        } else {
  ...
  proto.identify = function identify (opts, callback) {
    ...
    var args = makeArgs(self, { verbose: true }, opts);
  ...
  function makeArgs (self, val, opts) {
    var args = [
        'identify'
      , '-ping'
    ];
    ...
    if (val.format) {
      args.push('-format', opts.custom ? opts.custom : val.format);
    }
  ...
  helper.Format = function Format (o, val, opts) {
    o.format = opts.custom ? val : val.split(" ")[0];
  };
