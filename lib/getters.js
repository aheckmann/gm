...
module.exports = function (gm) {
   ...
   * - custom: specifying the custom -format argument (see man gm)
   */
  ...
  proto.custom = function identify (opts, callback) {
    if (!callback) callback = opts, opts = {};
    if (!callback) return this;
    ...
    var getter = 'custom'
      ...
      , self = this
      , cb = function () {
          self.removeAllListeners(getter);
          callback.apply(self, arguments)
        };

    self.on(getter, cb);

    // wrap to object parrern if passed as string
    ...
    var args = makeArgs(self, val, opts.pattern);
    self._exec(args, function (err, stdout, stderr, cmd) {
      if (err) {
        self.emit(getter, err, self.data[key], stdout, stderr, cmd);
        return;
  ...
  function makeArgs (self, val, pattern) {
    var args = [
        'identify'
      , '-ping'
    ];
