...
module.exports = function (gm) {
  ...
  function identifyPattern (pattern, opts, callback) {
    ...
    var args = makeArgs(self, { pattern: pattern });
    self._exec(args, function (err, stdout, stderr, cmd) {
      if (err) {
        return callback.call(self, err, self.data[key], stdout, stderr, cmd);
      }

      self.data[key] = (stdout||'').trim();
      callback.call(self, err, self.data[key]);
    });
