...
module.exports = function (proto) {
  ...
  // Handles mutiple drawRectangle() calls to render a dynamic number of rectangles on call (handover is an array of coords)
  proto.drawRectangleMulti = function drawRectangleMulti(coordsArray) {
    var i = 0
      , length = coordsArray.length
      , returnObjs = {};
    
    for(; i < length; i++) {
      returnObjs = this.drawRectangle(coordsArray[i][0], coordsArray[i][1], coordsArray[i][2], coordsArray[i][3], coordsArray[i][4], coordsArray[i][5]);
    }
