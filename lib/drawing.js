...
var validateValue = require('./utils').validateValue;
...
module.exports = function (proto) {
  ...
  proto.drawText = function drawText (x0, y0, text, gravity) {
    var arg = ["text " + x0 + "," + y0 + " " + escape(text)];

    gravity = validateValue(gravity, _gravities);
  ...
  proto.drawImage = function drawImage (operator, x0, y0, w, h, filename) {
    operator = validateValue(operator, _operators, "Over");
    return this.draw("image", operator, x0+","+y0, w+","+h, escape(filename));
  }
}
